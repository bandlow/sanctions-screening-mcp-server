...
const HitSchema = z
  .object({
    source: SOURCE_ENUM.describe(
      "Which watchlist this candidate is on — its provenance.",
    ),
    sourceLabel: z.string().describe("Human-readable name of the source list."),
    sourceEntryId: z
      .string()
      .describe(
        "The list's own entry ID — pass to sanctions_get_designation for the full record.",
      ),
    entityType: z
    ...
    primaryName: z
      .string()
      .describe("Primary published name of the designated entity."),
    matchedName: z
      .string()
      .describe("The specific name or alias string that matched the query."),
    matchedNameType: z
      .enum(["primary", "aka", "fka", "low-quality-aka"])
      .describe(
        "Provenance of the matched name: primary, a.k.a., f.k.a., or a low-quality a.k.a.",
      ),
    matchType: z
    ...
    queryTokenCoverage: z
      .object({
        ...
        total: z
          .number()
          .int()
          .describe("Total tokens in the normalized query."),
      })
      .optional()
      .describe(
  ...
  .describe(
    "One potential match — a candidate to verify, never a determination.",
  );
...
export const screenNameTool = tool("sanctions_screen_name", {
  ...
  input: z.object({
    name: z
      ...
      .min(1)
      .describe(
        "The name to screen (person, organization, vessel, or aircraft).",
      ),
    entityType: z
      .enum(["any", "person", "organization", "vessel", "aircraft", "unknown"])
      .default("any")
      .describe(
        'Restrict to one entity class, or "any" (default) to screen across all.',
      ),
    address: z
      .object({
        street: z
          ...
          .optional()
          .describe(
            "Optional street fragment that must appear in the designation address line.",
          ),
        houseNumber: z
          .string()
          .min(1)
          .optional()
          .describe(
            "Optional house-number fragment that must appear in the designation address line.",
          ),
        postalCode: z
          .string()
          .min(1)
          .optional()
          .describe(
            "Optional postal-code fragment that must appear in the designation address line.",
          ),
        country: z
          .string()
          .min(1)
          .optional()
          .describe(
            "Optional country fragment that must match the published address country or line.",
          ),
      })
      .optional()
      .describe(
        "Optional address filter. All provided address fields must match the same address row.",
      ),
    matchMode: z
      .enum(["strict", "fuzzy"])
      .default("strict")
    ...
    sources: z
      ...
      .optional()
      .describe(
        "Restrict to specific source lists. Omit to screen all loaded lists.",
      ),
    limit: z
      .number()
      .int()
  ...
  enrichment: {
    normalizedQuery: z
      .string()
      .describe("The name as the server folded it for matching."),
    matchModeUsed: z
      .string()
      .describe(
        "The match mode actually applied (strict may auto-upgrade to fuzzy on empty).",
      ),
    totalCount: z
      .number()
      .describe("Number of potential matches returned in this page."),
    totalAvailable: z
      .number()
      .describe(
    ...
    hasMore: z
      .boolean()
      .describe(
        "True when potential matches remain beyond this page — re-call with nextOffset.",
      ),
    nextOffset: z
      .number()
      .optional()
      .describe(
        "The offset to request next. Present only when hasMore is true.",
      ),
    notice: z
      .string()
      .optional()
  ...
  async handler(input, ctx) {
    ...
    return {
      hits: result.hits.map((h) => ({
        ...
        ...(h.queryTokenCoverage
          ? { queryTokenCoverage: h.queryTokenCoverage }
          : {}),
  ...
  format: (result) => {
    ...
    if (result.hits.length === 0) {
      ...
      for (const h of result.hits) {
        ...
        const coverStr = cov
          ? ` · covers ${cov.covered}/${cov.total} query tokens`
          : "";
        lines.push(
          `### ${h.primaryName} — ${h.matchType}${scoreStr}${coverStr}`,
        );
        lines.push(
