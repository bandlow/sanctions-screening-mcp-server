...
const IdentifierHitSchema = z
  .object({
    ...
    identifier: z
      .object({
        type: z
          .string()
          .describe("Identifier category as published by the source."),
        value: z
          .string()
          .describe("Identifier value as published by the source."),
        country: z
          .string()
          .optional()
  ...
  .describe(
    "One identifier match — a candidate to verify, never a determination.",
  );
...
export const searchIdentifierTool = tool("sanctions_search_identifier", {
  ...
  input: z.object({
    ...
    entityType: z
      ...
      .default("any")
      .describe(
        'Restrict to one entity class, or "any" (default) to search across all.',
      ),
    sources: z
      .array(SOURCE_ENUM)
      .optional()
      .describe(
        "Restrict to specific source lists. Omit to search all loaded lists.",
      ),
    limit: z
      .number()
      .int()
  ...
  output: z.object({
    hits: z
      ...
      .describe(
        "Published identifier matches, ordered by exactness and stable designation ID.",
      ),
    caveat: z
      .string()
      .describe(
  ...
  enrichment: {
    ...
    totalCount: z
      .number()
      .describe("Number of identifier matches returned in this page."),
    totalAvailable: z
      .number()
      .describe(
    ...
    hasMore: z
      .boolean()
      .describe("True when identifier matches remain beyond this page."),
    nextOffset: z
      .number()
      .optional()
      .describe(
  ...
  format: (result) => {
    ...
    if (result.hits.length === 0) {
      ...
      for (const hit of result.hits) {
        const country = hit.identifier.country
          ? ` (${hit.identifier.country})`
          : "";
        lines.push(`### ${hit.primaryName} — ${hit.matchType}`);
        lines.push(
          `**Identifier:** ${hit.identifier.type}: ${hit.identifier.value}${country}`,
        );
        lines.push(
        ...
        if (hit.designationDate)
          lines.push(`**Designated:** ${hit.designationDate}`);
        lines.push("");
      }
    }
