...
export interface ScreenNameOptions {
  ...
  address?: {
    /** Street fragment to require in the published address line. */
    street?: string;
    /** House-number fragment to require in the published address line. */
    houseNumber?: string;
    /** Postal-code fragment to require in the published address line. */
    postalCode?: string;
    /** Country fragment to require in address country/full line. */
    country?: string;
  };
...
export class ScreeningService {
  ...
  async screenName(
    ...
    const strictHits = this.applyAddressFilter(
      handle,
      strict.results,
      opts.address,
    );
    ...
    const fuzzyFiltered = this.applyAddressFilter(
      handle,
      fuzzyHits,
      opts.address,
    );
    ...
    const merged = this.mergeHits(strictHits, fuzzyFiltered);
    ctx.log.debug("Fuzzy screening complete", {
      normalizedQuery,
      strictCount: strictHits.length,
      fuzzyCount: fuzzyFiltered.length,
      minScore,
    });
    return {
  ...
  private applyAddressFilter(
    handle: SqliteHandle,
    hits: ScreeningHit[],
    address: ScreenNameOptions["address"],
  ): ScreeningHit[] {
    const normalized = {
      street: fold(address?.street ?? ""),
      houseNumber: fold(address?.houseNumber ?? ""),
      postalCode: fold(address?.postalCode ?? ""),
      country: fold(address?.country ?? ""),
    };
    const hasFilter =
      normalized.street.length > 0 ||
      normalized.houseNumber.length > 0 ||
      normalized.postalCode.length > 0 ||
      normalized.country.length > 0;
    if (!hasFilter || hits.length === 0) return hits;
    ...
    const rows = handle
      .prepare<{
        id: string;
        payload: string;
      }>(`SELECT id, payload FROM designation WHERE id IN (${placeholders})`)
      .all(...hits.map((h) => h.designationId));
    ...
    return hits.filter((hit) => {
      ...
      return payload.addresses.some((addr) => {
        const full = fold(addr.full);
        const country = fold(addr.country ?? "");

        if (normalized.street && !full.includes(normalized.street))
          return false;
        if (normalized.houseNumber && !full.includes(normalized.houseNumber))
          return false;
        if (normalized.postalCode && !full.includes(normalized.postalCode))
          return false;
        if (
          normalized.country &&
          !country.includes(normalized.country) &&
          !full.includes(normalized.country)
        ) {
          return false;
        }
    ...
    for (const row of rows) {
      ...
      for (const identifier of payload.identifiers) {
        ...
        const exactIdentifierToken =
          ...
          tokenize(value).includes(normalizedQuery);
