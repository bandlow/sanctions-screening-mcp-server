...
describe("buildFtsMatch", () => {
  ...
  it("can build prefix-token matches when requested", () => {
    expect(buildFtsMatch("Ivan Vol", { prefixTokens: true })).toBe(
      '"ivan"* AND "vol"*',
    );
  });
...
describe("lengthRatio", () => {
  ...
  it("is the shorter length over the longer, order-independent", () => {
    ...
    expect(lengthRatio("nicolas", "nicolas maduroo moros")).toBeCloseTo(
      7 / 21,
      6,
    );
    expect(lengthRatio("nicolas maduroo moros", "nicolas")).toBeCloseTo(
      7 / 21,
      6,
    );
  });
  ...
  it("drops below the whole-string guard when a short token merely prefixes a long query", () => {
    ...
    expect(jaroWinkler("nicolas maduroo moros", "nicolas")).toBeGreaterThan(
      0.85,
    );
  ...
  it("stays high for spacing/concatenation variants (near-equal length)", () => {
    ...
    expect(
      lengthRatio("vanderbergshipping", "van der berg shipping"),
    ).toBeGreaterThan(0.8);
  });
});
