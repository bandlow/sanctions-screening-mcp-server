 ...
 */
const ctxFor = <const E extends readonly ErrorContract[] | undefined>(
  errors: E,
...
describe("screening tools (seeded)", () => {
  ...
  it("screen_name returns scored hits and the decision-support caveat", async () => {
    ...
    expect(result.caveat).toMatch(
      /screening aid|not a compliance determination/i,
    );
  ...
  it("screen_name applies request-body address filters to matching", async () => {
    const input = screenNameTool.input.parse({
      name: "FTC LLC",
      address: {
        street: "Commerce Way",
    ...
    expect(
      result.hits.find((h) => h.sourceEntryId === "FX-2002"),
    ).toBeDefined();
  });

  it("screen_name input accepts entityType unknown", () => {
    const parsed = screenNameTool.input.parse({
      name: "anyone",
      entityType: "unknown",
    });
    expect(parsed.entityType).toBe("unknown");
  });

  it("get_designation returns the full record with the caveat", async () => {
    const input = getDesignationTool.input.parse({
      source: "ofac_sdn",
      entryId: "FX-1001",
    });
  ...
  it("search_identifier finds published identifiers by exact value or contained number", async () => {
    const imo = await searchIdentifierTool.handler(
      searchIdentifierTool.input.parse({
        identifier: "1234567",
        identifierType: "IMO",
      }),
  ...
  it("list_sources reports counts, readiness, and licenses", async () => {
    ...
    expect(result.sources.find((s) => s.code === "gleif")?.license).toMatch(
      /CC0/i,
    );
  });
  ...
  it("resolve_entity returns ranked LEI candidates", async () => {
    const input = resolveEntityTool.input.parse({
      name: "Fictional Trading Company LLC",
    });
...
describe("query-token coverage on both client surfaces (issue #15)", () => {
  ...
  it("screen_name omits coverage for an exact hit on both surfaces", async () => {
    ...
    expect(result.hits.every((h) => h.queryTokenCoverage === undefined)).toBe(
      true,
    );
  ...
  it("screen_name keeps coverage on every hit of a capped page", async () => {
    ...
    expect(renderFormat(screenNameTool, result)).toContain(
      "covers 2/3 query tokens",
    );
  });

  it("rejects an out-of-range minScore before the handler ever runs", () => {
    expect(() =>
      screenNameTool.input.parse({ name: "Ivan Volkow", minScore: 2 }),
    ).toThrow();
    expect(() =>
      resolveEntityTool.input.parse({ name: "Fictional", minScore: -1 }),
    ).toThrow();
  });
  ...
  it("resolve_entity omits coverage for an exact match on both surfaces", async () => {
    ...
    expect(renderFormat(resolveEntityTool, result)).not.toContain(
      "query tokens",
    );
  });
});
