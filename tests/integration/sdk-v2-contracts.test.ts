...
toolContractSuite(listSourcesTool, {
  success: [
    { name: "reports every loaded source with its provenance", input: {} },
  ],
});
  ...
  success: [
    {
      ...
      assert: (result) => {
        ...
        expect(
          structured.nodes.find((node) => node.lei === PARENT_LEI)?.depth,
        ).toBe(1);
        ...
        const root = structured.nodes.find(
          (node) => node.lei === SUBSIDIARY_LEI,
        );
        expect(root?.sanctionsHits?.[0]?.primaryName).toBe(
          "Fictional Trading Company LLC",
        );
        expect(
          structured.nodes.find((node) => node.lei === PARENT_LEI)
            ?.sanctionsHits,
        ).toEqual([]);
        expect(renderContent(result.content)).toContain(
...
describe("advertised tool schemas", () => {
  it("closes every root input and reserves `error` for the failure envelope", () => {
    for (const definition of allToolDefinitions) {
      const inputSchema = z.toJSONSchema(definition.input) as Record<
        string,
        unknown
      >;
...
describe("resource cache hints", () => {
  const hintFor = (uriTemplate: string) =>
    allResourceDefinitions.find(
      (definition) => definition.uriTemplate.toString() === uriTemplate,
    )?.cacheHint;
