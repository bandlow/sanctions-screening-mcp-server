...
describe("text matcher fuzz invariants", () => {
  it("keeps folding, phonetics, and similarity bounded for hostile Unicode", () => {
    ...
    for (const value of values) {
      ...
      expect(jaro(normalized, reversed)).toBeCloseTo(
        jaro(reversed, normalized),
        12,
      );
      ...
      if (match)
        expect(
          match.split(" AND ").every((token) => /^"[a-z0-9]+"$/.test(token)),
        ).toBe(true);
    }
  });
});
...
describe("ingest parser fuzz invariants", () => {
  ...
  it("drops truncated streaming records rather than emitting partial entities", async () => {
    ...
    expect(
      await collect(streamLeiLevel1FromText(chunks(truncated, 1))),
    ).toHaveLength(0);
    expect(
      await collect(streamLeiLevel1FromText(chunks(truncated, 7))),
    ).toHaveLength(0);
  });
  ...
  it("parses the same payload twice into identical records, entry ids included", () => {
    const cases: [
      label: string,
      expectedIds: string[],
      parse: () => NormalizedDesignation[],
    ][] = [
      [
        "ofac standard",
        ["77"],
        () => parseOfac(parseXml(MIXED_OFAC_STANDARD_XML), "ofac_sdn"),
      ],
      [
        "ofac advanced",
        ["2674"],
        () => parseOfac(parseXml(MIXED_OFAC_ADVANCED_XML), "ofac_sdn"),
      ],
